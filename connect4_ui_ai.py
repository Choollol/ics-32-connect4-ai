...
LOOKAHEAD_MOVE_THRESHOLDS = [(5, 6), (8, 5), (12, 4), (20, 4)]
...
def faster_win_check(game_state: connect4.GameState, columnCount: int, rowCount: int):
    ...
    for col in range(columnCount):
        for row in range(rowCount):
            if _winning_sequence_begins_at(game_state.board, col, row, columnCount, rowCount):
...
def simulate_move(gameState: connect4.GameState, move: int, movesRemaining: int, column: int, lookaheadRange) -> int:
    ...
    while queue:
        ...
        if faster_win_check(front[0], column_count, row_count) != connect4.EMPTY:
            ...
            total_score += get_single_score(connect4.winner(front[0])) * front[2] ** front[2]
            continue
        if front[1] == DROP:
...
def ai_move(gameState: connect4.GameState) -> tuple[int, int]:
    ...
    for pair in LOOKAHEAD_MOVE_THRESHOLDS:
        if column_count <= pair[0]:
            lookahead_moves = pair[1]
            break

    column = random.randint(1, column_count)
    ...
    for col in range(1, column_count + 1):
        if gameState.board[col - 1][-1] != connect4.EMPTY:
            lookaheadRangeLeft = col - LOOKAHEAD_EDGE_RADIUS if col - LOOKAHEAD_EDGE_RADIUS >= 1 else 1
            break
            
    for col in range(column_count, 0, -1):
        if gameState.board[col - 1][-1] != connect4.EMPTY:
            lookaheadRangeRight = col + LOOKAHEAD_EDGE_RADIUS if col + LOOKAHEAD_EDGE_RADIUS <= column_count else column_count
            break
    ...
    """ for col in range(1, column_count + 1):
        if scores[col - 1] == DEFAULT_MAX_SCORE:
            continue
        score = scores[col - 1]
        print(F"Column {col}'s score: {score}")
        if score > max_score:
    ...
    """ for col in range(1, column_count + 1):
        if not is_move_valid(gameState, DROP, col):
            continue
        score = simulate_move(gameState, DROP, lookahead_moves, col, column_count + 1)
        print(f"Column {col}'s score: {score}")
